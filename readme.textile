...
First let's see how this actually works. We've created a macro called <code>quick-snippet</code> which loads and analyzes a widget so it can produce the right code. With <code>quick-snippet</code> instead of writing out the whole template you can write the following:
...
It's important to be able to expand macros in order to debug them. Start your REPL if it's not already up and running:
...
Now try the following at your REPL:
...
Clojure macros are very, very similar to Common Lisp macros. I whole heartedly recommend looking at Paul Graham's On Lisp for a more in-depth treatment.